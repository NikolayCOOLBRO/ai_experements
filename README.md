...
8. В отдельной панели `Действия агента` можно посмотреть, какие сообщения попали в prompt и как обновлялось summary.

Дефолтного агента нет. Чат не создается автоматически: перед первым запуском нужно явно создать его в UI.

Интерфейс позволяет отдельно скрывать панель агентов и панель действий агента, чтобы расширять область чата.
...
backend/
  ...
  agent_store.py   SQLite storage for agents, chats, summaries and traces
  requirements.txt Python dependencies

frontend/
  src/App.tsx      Agent workspace UI with chat, agents panel and trace panel
